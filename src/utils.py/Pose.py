...
class Pose3D(Pose):
    ...
    def oplus(AxB, BxC):
        """
        Given a Pose3D object *AxB* (the self object) and a Pose3D object *BxC*, 
        it returns the Pose3D object *AxC*.
        ...
        AxC_y = AxB[1][0] + (BxC[0][0] * sin(AxB[2][0])) + (BxC[1][0] * cos(AxB[2][0]))
        AxC_yaw = AxB[2][0] + BxC[2][0]
        
        AxC = Pose3D(np.array([[AxC_x], [AxC_y], [AxC_yaw]]))
    ...
    def ominus(AxB):
        ...
        """
        # TODO: To be completed by the student
        BxA_x = (-AxB[0][0] * cos(AxB[2][0]) - (AxB[1][0] * sin(AxB[2][0])))
        BxA_y = (AxB[0][0] * sin(AxB[2][0])) - (AxB[1][0] * cos(AxB[2][0]))
        BxA_yaw = -AxB[2][0]
                 
        BxA = Pose3D(np.array([[BxA_x], [BxA_y], [BxA_yaw]]))
