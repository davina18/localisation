...
def c2p(c):  # cartesian to spherical conversion 
    ...
    """
    if c is None or c.shape != (2, 1):
        raise ValueError("Invalid input. Expected a (2,1) numpy array representing Cartesian coordinates.")
    
    x, y = c[0, 0], c[1, 0]
    rho = np.sqrt(x**2 + y**2)
    theta = np.arctan2(y, x)
    
    polar_data = np.array([[rho], [theta]])
    return PolarFeature(polar_data)  # Return PolarFeature instance
